...
// BUAT FILTER TERPISAH UNTUK MASING-MASING TABEL
    ...
    <MemberLayouts backRoute="member.training.index">
        ...
        <div class="py-3 space-y-6">
                ...
                <div class="p-5">
                    ...
                    <div v-else-if="schedules.length > 0">
                        <div class="relative w-full md:w-64 mb-4">
                            <input v-model="upcomingSchedulesFilter['global'].value" type="text"
                        ...
                        <DataTable :value="schedules" v-model:filters="upcomingSchedulesFilter" paginator :rows="10"
                            dataKey="id" :loading="loading" stripedRows removableSort class="p-datatable-sm" :pt="{
                            ...
                            <!-- Status -->
                            <Column field="status" header="Status" class="w-32" :pt="{
                                columnHeaderContent: { class: '!justify-center' },
                            }">
                                <template #body="{ data }">
                                    <div class="text-center rounded-lg px-3 py-1 w-min mx-auto whitespace-nowrap border shadow text-xs"
                                        :class="[
                                            data.status === 'Hadir'
                            ...
                            <!-- Alasan -->
                            <Column field="reason" header="Alasan" class="w-64" :pt="{
                                columnHeaderContent: { class: '!justify-center' },
                            }">
                                <template #body="{ data }">
            ...
            <div class="bg-white rounded-lg shadow">
                ...
                <div class="p-5">
                    <div class="relative w-full md:w-64 mb-4">
                        <input v-model="attendanceHistoryFilter['global'].value" type="text"
                            placeholder="Cari jadwal..."
                            class="border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-0 shadow-sm w-full pl-10" />
                        <span class="absolute left-3 top-2.5 text-gray-400">
                            <i class="fas fa-search"></i>
                    ...
                    <DataTable :value="attendanceHistory" v-model:filters="attendanceHistoryFilter" paginator :rows="10"
                        dataKey="id" :loading="loading" stripedRows removableSort class="p-datatable-sm" :pt="{
                            thead: { class: 'text-sm font-light' },
                            tbody: { class: 'text-sm font-light' },
                            pcPaginator: { content: { class: 'text-xs' } },

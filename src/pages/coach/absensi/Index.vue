...
import { Column, DataTable, Dialog, Toast, useToast } from 'primevue'
...
const filteredAttendances = computed(() => {
  ...
  return mergedAttendances.value.filter((member) => member.name.toLowerCase().includes(query))
})
...
<template>
  <CoachLayouts>
    ...
    <div class="py-3 space-y-3">
      <!-- HEADER -->
      <div
        class="rounded-lg bg-white shadow px-5 py-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4"
      >
        <div class="flex flex-col md:flex-row items-start md:items-center gap-3 w-full md:w-auto">
          <div class="w-full md:w-64">
            ...
            <select
              ...
              class="border border-gray-300 rounded-lg px-3 py-2 w-full shadow-sm focus:ring-2 focus:ring-piper-500 focus:border-piper-500"
            >
          ...
          <div class="w-full md:w-64">
            ...
            <div class="relative">
              <input
                ...
                class="border border-gray-300 rounded-lg px-3 py-2 pl-9 w-full shadow-sm focus:ring-2 focus:ring-piper-500 focus:border-piper-500"
              />
        ...
        <button
          ...
          class="w-full md:w-auto flex items-center justify-center gap-2 bg-rhino-700 hover:bg-rhino-900 text-white px-4 py-2.5 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
        >
        ...
        >
            ...
            <template #body="{ data }">
              <div class="flex justify-between items-center">
                ...
                <button
                  ...
                  :class="[
                    'text-xs rounded-md px-3 py-1.5 transition-all duration-200 font-medium',
                    data.status === 'Hadir'
          ...
          <Column field="status" header="Status" class="w-32 text-center" sortable>
            <template #body="{ data }">
              <span
                :class="[
                  'px-3 py-1.5 rounded-full text-xs font-medium inline-block min-w-24',
                  data.status === 'Hadir'
    ...
    >
      <div v-if="selectedMember" class="space-y-4">
        ...
        <div>
          ...
          <select
            v-model="formStatus"
            class="border border-gray-300 rounded-lg w-full px-3 py-2.5 text-sm focus:ring-2 focus:ring-piper-500 focus:border-piper-500"
            :disabled="saving"
          >
        ...
        <div class="flex justify-end gap-2 pt-2">
          ...
          <button
            ...
            class="px-4 py-2 text-sm rounded-lg bg-piper-600 text-white hover:bg-piper-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
          >
            <i v-if="saving" class="pi pi-spin pi-spinner" style="font-size: 0.9rem"></i>
    ...
    >
      <div class="space-y-4">
        <div class="bg-piper-50 border border-piper-200 rounded-lg p-3">
          <p class="text-sm text-piper-700">
            <i class="pi pi-info-circle mr-1"></i>Pastikan jadwal latihan sudah dipilih sebelum scan
            QR code.
          </p>
        </div>
        ...
        <div class="flex justify-end gap-2 pt-2">
          <button
            ...
            class="px-4 py-2 text-sm rounded-lg border border-gray-300 bg-white text-gray-700 hover:bg-gray-50 transition-colors"
          >
            Tutup
          </button>
        </div>
